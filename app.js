...
function renderMacroView() {
  ...
  let sacCalculado = 0;
  if (esSacMonth) {
    const sacB = (data.sacBrian !== null && data.sacBrian !== undefined) ? data.sacBrian : (data.salaryBrian / 2);
    const sacV = (data.sacVirginia !== null && data.sacVirginia !== undefined) ? data.sacVirginia : (data.salaryVirginia / 2);
    ...
    const sacCardDisplay = document.getElementById('macro-sac-card-display');
    if (sacCardDisplay) sacCardDisplay.textContent = formatearMoneda_(sacCalculado);
  }
...
function renderMacroIncomesList_(data, sac, premio, deudas, usdRate) {
  ...
  const pBrian = data.premioBrian !== undefined ? data.premioBrian : (data.premio || 0);
  const pVirginia = data.premioVirginia || 0;

  let html = '';
  html += '<div class="flex justify-between py-1.5 border-b border-zinc-100"><span>Sueldo Brian:</span><span class="font-bold">' + formatearMoneda_(data.salaryBrian + pBrian) + (pBrian !== 0 ? (' <span class="text-xs font-normal text-zinc-400">(' + (pBrian > 0 ? '+' : '') + formatearMoneda_(pBrian) + ')</span>') : '') + '</span></div>';
  html += '<div class="flex justify-between py-1.5 border-b border-zinc-100"><span>Sueldo Virginia:</span><span class="font-bold">' + formatearMoneda_(data.salaryVirginia + pVirginia) + (pVirginia !== 0 ? (' <span class="text-xs font-normal text-zinc-400">(' + (pVirginia > 0 ? '+' : '') + formatearMoneda_(pVirginia) + ')</span>') : '') + '</span></div>';

  if (pBrian !== 0 || pVirginia !== 0) {
    html += '<div class="flex justify-between py-1 border-b border-zinc-100 text-xs text-zinc-500"><span>Ajuste Total Sueldos:</span><span class="font-semibold">' + (premio > 0 ? '+' : '') + formatearMoneda_(premio) + '</span></div>';
  }

  if (sac > 0) {
    html += '<div onclick="openSalaryAdjustModal()" class="flex justify-between py-1.5 border-b border-zinc-100 text-amber-800 font-medium cursor-pointer hover:bg-amber-50/50 rounded px-1 transition-colors" title="Tocar para editar Aguinaldo (SAC)"><span>SAC (Aguinaldo) <span class="text-xs text-amber-600">✎</span>:</span><span class="font-bold">' + formatearMoneda_(sac) + '</span></div>';
  }
...
function openSalaryAdjustModal() {
  ...
  // Pre-llenar con el Total Cobrado en Bolsillo (base + delta). Si no hay delta previo, muestra el sueldo base o el valor actual cobrado.
  const totalCobradoBrian = baseBrian + prizeBrian;
  const totalCobradoVirginia = baseVirginia + prizeVirginia;

  setMoneyValue('salary-adjust-input-brian', totalCobradoBrian);
  setMoneyValue('salary-adjust-input-virginia', totalCobradoVirginia);
  ...
  if (esSacMonth) {
    const sacB = (data.sacBrian !== null && data.sacBrian !== undefined) ? data.sacBrian : (baseBrian / 2);
    const sacV = (data.sacVirginia !== null && data.sacVirginia !== undefined) ? data.sacVirginia : (baseVirginia / 2);
    setMoneyValue('salary-adjust-sac-input-brian', sacB);
    setMoneyValue('salary-adjust-sac-input-virginia', sacV);
